...
async def init_loop(ip, port, strip):
    def set_preset_osc(self, address: str, val: str, *args):
        self.set_preset(val)
        print(f"Preset set to {val}") 
    server = osc_server.AsyncIOOSCUDPServer(
        (ip, port), strip.dispatcher, asyncio.get_event_loop())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--preset",
                        default=None, help="The preset to use")
    ...
    asyncio.run(init_loop(args.ip, args.port, my_strip))
